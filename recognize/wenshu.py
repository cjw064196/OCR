...
# -*- coding: utf-8 -*-
"""
Created on Mon May 21 10:01:15 2018
...
class Ocr():
    def __init__(self):
        ...
        self.width = 132
        self.heigth = 40
        self.X = tf.placeholder(tf.float32, [None, 132*40])
        		# 输入数据Y占位符
    ...
    def crack_captcha_cnn(self, w_alpha=0.01, b_alpha=0.1):
        ...
        w_d = tf.Variable(w_alpha*tf.random_normal([5*17*64, 1024]))
...
def crack_captcha(captcha_image):
    ...
    with tf.Session() as sess:
        saver.restore(sess, tf.train.latest_checkpoint(dir_path + "/wenshu/model"))
...
if __name__ == '__main__':
    dir_path = os.path.dirname(os.path.realpath(__file__))
    filename = dir_path + '/wenshu/1.png'
