...
# -*- coding: utf-8 -*-
"""
Created on Thu May  3 14:32:50 2018
...
class Ocr():
    ...
    def get_imgs(self, rate = 0.2):
		...
		"""
		# 读取图片
		imgs = os.listdir(self.train_data_path)
		# 打乱图片顺序
		random.shuffle(imgs)
 ...
 '''   
    ...
    def __init__(self):
		...
		# 读取数据集
        self.train_imgs,train_labels=self.get_imgs_train()
    ...
    def get_next_batch(self, train_flag=True, batch_size=100):
		...
		# 从测试集获取数据
            ...
            for index, test in enumerate(tests):
                img = np.mean(cv2.imread(self.test_data_path + test), -1)#灰度化图片
				# 将多维降维1维
                batch_x[index,:] = img.flatten() / 255
            for index, label in enumerate(labels):
                batch_y[index,:] = self.text2vec(label)			
    ...
    def train_crack_captcha_cnn(self):
        ...
        #diff = tf.nn.sigmoid_cross_entropy_with_logits(output, self.Y)
